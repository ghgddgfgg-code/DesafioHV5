...
function registerSW(){
  ...
  navigator.serviceWorker.register('/DesafioHV5/service-worker.js').then(reg=>{
  ...
  slot.innerHTML=`
    <div class="drawer-user">
      ...
      <div class="drawer-user-info">
        <div class="drawer-user-name">${user.displayName||'Usuario'}</div>
...
function renderProfile(){
  ...
  // Nombre de perfil: usar nombre de Google si hay sesión
  ...
  if(currentUser){
    cloudSlot.innerHTML=`<div class="profile-cloud-badge">☁️ Sincronizado con Google</div>`;
  } else {
...
function initApp(){
  ...
  newActDays=[...ALL_DAYS];buildDayPicker('f-days',newActDays,null);
  render();initParticles();renderNotifCard();
